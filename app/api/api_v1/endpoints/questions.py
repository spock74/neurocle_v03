...
async def send_question_route(
    ...
    try:
        # Garantir que o assistant_id do path corresponda ao do payload
        if question.assistant_id != assistant_id:
            raise HTTPException(
                status_code=400,
                detail="assistant_id in path must match assistant_id in request body"
            )
